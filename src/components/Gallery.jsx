...
export default function Gallery() {
  ...
  return (
    <section id="gallery" className="w-full bg-slate-950">
      <div className="max-w-7xl mx-auto px-6 py-24 md:py-32">
        <div className="flex items-end justify-between mb-10">
          <h3 className="text-3xl md:text-5xl font-semibold text-white tracking-tight">Photo & Video Gallery</h3>
          <p className="text-slate-400 max-w-md hidden md:block">A curated selection of moments: stage craft, atmosphere, and energy. Subtle motion and light capture the essence of DELUX.</p>
        </div>
      ...
      <AnimatePresence>
        {active && (
          <motion.div className="fixed inset-0 z-[60] bg-black/80 backdrop-blur-sm flex items-center justify-center p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setActive(null)}>
            <motion.img src={active} alt="Full" className="max-h-[85vh] w-auto rounded-2xl border border-white/10" initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} />
          </motion.div>
        )}
      </AnimatePresence>
    </section>
